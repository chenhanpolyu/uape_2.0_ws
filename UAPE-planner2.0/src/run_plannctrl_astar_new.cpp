...
#include <Tools.h>
#include <path_searching/include/path_searching/astar.h>
#include <mlmap.h>
#include <time.h>
...
int main(int argc, char **argv)
{
  ...
  double ball_pass_time = 2.0, dyn_pass_time = 0.01;
  ...
  Astar_path_finder_.reset(new Astar);
  Astar_path_finder_->setParam(nh);
  Astar_path_finder_->init();
  ...
  while (nh.ok()) // main loop
  {
    ...
    if (mapPtr->map_updated || RosMsg.dyn_update)
    {
      ...
      if (!RosMsg.dyn_update && RosMsg.dynobs_pointer->ball_number > 0 && ros::Time::now().toSec() - RosMsg.dynobs_pointer->ball_time_stamp > ball_pass_time) // for bag sim// RosMsg.dynobs_pointer->ball_number>0 && (RosMsg.dynobs_pointer->ballvel[0](0) > -0.2)||
      {
        if (if_debug)
          cout << "dyn ball time out: " << RosMsg.dynobs_pointer->ballvel[0] << endl;
        RosMsg.dynobs_pointer->clear_ball();
        ball_time_out = true;
      }
      if (!RosMsg.dyn_update && RosMsg.dynobs_pointer->dyn_number > 0 && ros::Time::now().toSec() - RosMsg.dynobs_pointer->time_stamp > dyn_pass_time) // for bag sim// RosMsg.dynobs_pointer->ball_number>0 && (RosMsg.dynobs_pointer->ballvel[0](0) > -0.2)||
      {
        RosMsg.dynobs_pointer->clear_dyn();
      ...
      // bool if_old_kinopath_collide = !Astar_path_finder_->checkOldPath(waypoints, ct_pos, false);
      ...
      // }
      if ((if_time_out | (!if_safe | if_initial)) && FE_status != Astar::GOAL_CLOSE)
      {
        
        chrono::high_resolution_clock::time_point tic = chrono::high_resolution_clock::now();
        Astar_path_finder_->setEnvironment(mapPtr);
        // Astar_path_finder_->setEnvironment(RosMsg.obs_pointer, RosMsg.dynobs_pointer, camera_vertex, gbbox_o, gbbox_l);
        ...
        FE_status = Astar_path_finder_->search(ct_pos, goal);
        // last_path_t = ros::Time::now().toSec();
        while (FE_status == Astar::GOAL_OCC)
        {
          cout << "[Astar replan]: Goal occluded:\n"
        ...
        if (FE_status == Astar::NO_PATH)
        {
          
            if_reach = false;
            cout << "[Astar replan]: Can't find path. Please restart" << endl;
            return 0;
        }
        else if (FE_status == Astar::GOAL_OCC)
        {
          return 0;
        }

        double compTime = chrono::duration_cast<chrono::microseconds>(chrono::high_resolution_clock::now() - tic).count() * 1.0e-3;
        cout << "Astar path planning finished! time cost (ms)： " << compTime << endl;
        if (FE_status == Astar::REACH_END)
        {
          waypoints = Astar_path_finder_->getPath();
        }
        else
        {
          waypoints.clear();
      ...
      {
        // cout << "[Astar replan]: Old Astar path is safe" << endl;
      }
      ...
      if ((if_safe && !if_initial && !((!last_if_reach) && if_reach) && !ball_time_out) | 
           (FE_status == Astar::GOAL_CLOSE && !if_initial)) //&& (timee+sfck_t < planner.total_t || planner.total_t <sfck_t)
      {
      ...
      else
      {
        ...
        if (planner.Max_traj_vel > max(2.0, planner.config.velMax) + 1.0)
        {
    ...
    else
    {
      ...
      desire_psi = planner.getYaw(planner.total_t + traj_last_t.toSec(), mapPtr);

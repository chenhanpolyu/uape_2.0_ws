...
private:
  ...
  /* ---------- parameter ---------- */
  /* search */
  double max_tau_, init_max_tau_, S_r, S_r_init;
  double max_vel_, max_acc_, max_acc_init, acc_sample_num_,acc_sample_num_init;
  ...
  inline void checkDynSafety(const Eigen::Vector3d &query_pt, const double current_time, bool &dyn_occ);
  inline bool line_collide(Eigen::Vector3d &p1, Eigen::Vector3d &p2, float max_len = INFINITY);
  double dist_sqr;
...
public:
  ...
  void init();
  void reset();
  bool checkOldPath(vector<Eigen::Vector3d> &point_set, Eigen::Vector3d ct_pos, bool if_check = true);
  ...
  void setEnvironment(mlmap::Ptr map, dynobs_tmp *dynamic_obs, Eigen::Matrix<double, 3, 5> &camera_vertex, vector<double> glbox_o, vector<double> glbox_l);

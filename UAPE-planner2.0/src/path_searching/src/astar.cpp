...
int Astar::search(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt, bool dynamic, double time_start)
{
  ...
  if (MLmap->getInflateOccupancy(end_pt) == MLmap->OCCUPIED)
  {
    return GOAL_OCC;
  }
  else if ((start_pt - end_pt).norm() < 0.5)
  {
    return GOAL_CLOSE;
  }
  /* ---------- initialize ---------- */
  // cout<<"begin search-00!"<<endl;
    ...
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_)
        {
          ...
          /* inside map range */
          if (pro_pos(0) <= origin_[0] || pro_pos(0) >= origin_[0] + map_size_3d_[0] || pro_pos[1] <= origin_[1] ||
              pro_pos(1) >= origin_[1] + map_size_3d_[1] || pro_pos[2] <= origin_[2] ||
              pro_pos(2) >= origin_[2] + map_size_3d_[2])
          {
            // cout << "outside map" << endl;
            continue;
          }
          // cout << "check cost 000!" << endl;
          /* not in close set */
          Eigen::Vector3i pro_id = posToIndex(pro_pos);

          // int pro_t_id = timeToIndex(pro_t);
          // cout << "check cost 00!" << endl;
          ...
          // cout << "check cost 0!" << endl;
          if (MLmap->getInflateOccupancy(pro_pos) == MLmap->OCCUPIED)
          {
            continue;
          }
...
void Astar::setParam(ros::NodeHandle &nh)
{
  ...
  // nh.getParam("search/margin", margin_);
  nh.getParam("search/allocate_num", allocate_num_);
  nh.getParam("GlobalBox_min", origin_);
  nh.getParam("GlobalBox_size", map_size_3d_);

  tie_breaker_ = 1.0 + 1.0 / 10000;
  origin_vec << origin_[0], origin_[1], origin_[2];
  cout << "origin_: " << origin_vec.transpose() << endl;
  cout << " resolution_: " << resolution_ << " allocate_num: " << allocate_num_ << " lambda_heu: " << lambda_heu_ << endl;
...
bool Astar::line_collide(const Eigen::Vector3d &p1, const Eigen::Vector3d &p2)
{
  double dist = (p1 - p2).norm();

  int num = static_cast<int>(dist / resolution_);
  for (auto i = 1; i <= num; i++)
  {
    if (MLmap->getInflateOccupancy(p1 + (p2 - p1) * i / num) == MLmap->OCCUPIED)
      return true;
  }
  if (MLmap->getInflateOccupancy(p2)== MLmap->OCCUPIED)
    return true;
  return false;
}
...
std::vector<Eigen::Vector3d> Astar::getPath()
{
  ...
  int size = path_nodes_.size();
  for (int i = 0; i < size-1; i++)
  {
    if (i == 0 || (i > 0 && line_collide(path_nodes_[i - 1]->position, path_nodes_[i + 1]->position)))
      path.push_back(path_nodes_[i]->position);
    else if (i > 0)
    {
      path_nodes_.erase(path_nodes_.begin() + i);  //i th node is not needed
      size--;
      i--;
    }
  }
  path.push_back(path_nodes_.back()->position);
  return path;
}
...
void Astar::setEnvironment(const mlmap::Ptr map)
{
  this->MLmap = map;
}
...
{
  Vector3i idx = ((pt - origin_vec) * inv_resolution_).array().floor().cast<int>();
